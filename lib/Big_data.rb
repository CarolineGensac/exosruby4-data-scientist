...
def shorter(handle)
    petit = handle.sort {|l, r| l.length <=> r.length}
    puts "#{petit[0]}"
end
...
#4Combien commencent par une majuscule

def maj(handle)
   grande = handle.count {|majus| majus[1][/[A-Z]/]}
   puts "#{grande} handle qui commencent par une majuscule"
end  
...
def taille(handle)
    taille = handle.sort {|l, r| l.length <=> r.length}
    puts "#{taille[0]}"
end

puts "---------------------------------"

#7 Quelle est la position dans l'array de la personne 

def select_epenser(handle)
    find_epenser= handle.index("@epenser")
    puts "Epenser se trouve a la position #{find_epenser}"
end


#8 Sors-moi une répartition des handle par taille de ces derniers

def repartition(handle)
    taille_handle = handle.count {|size| size[1][/[A-Z]/]}
